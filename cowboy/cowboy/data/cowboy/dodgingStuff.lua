...
function onCreate()
	...
	if isMobile then
		setTextString('dodgeWarning', 'PRESS THE HEALTHBAR TO DODGE')
...
function onUpdate(elapsed)
    ...
    if not downscroll then
	    ...
	    if getMouseX('hud') > 294 and getMouseX('hud') < 948 and getMouseY('hud') < 194 and mouseClicked() and isMobile then
	    	cowboyDodge()
	    end
	end

	--debugPrint('x: '.. getCameraFollowX())
	--debugPrint('y: '.. getCameraFollowY())
